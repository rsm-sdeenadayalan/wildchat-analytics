...
**Interfaces:**
- Produces in `app.js`:
  - `const AGGREGATES = ["volume_daily_model","volume_weekly_country","volume_weekly_language","intensity_weekly","depth_by_model","intent_weekly","intent_by_model","intent_by_language","friction_by_intent_model","friction_weekly","data_quality_weekly"]` (must equal `loupe.stages.metrics.METRICS`).
...
export async function renderIntensity(conn) {
  ...
  view.append(card("Week-over-week return rate", "Share of pseudo-users active in a week who are active again the following week. The final week is always 0 because there is no following week.",
    Plot.plot({ width: w, height: 260, marginLeft: 50, y: { label: "return rate", grid: true, percent: true }, x: { label: null },
      marks: [Plot.lineY(weekly.slice(0, -1), { x: "week", y: "return_rate", stroke: css("--c1"), tip: true })] })));
...
export async function renderQuality(conn, meta) {
  ...
  view.append(card("Top countries (cells under the minimum size suppressed)", `Any week × country cell with fewer than ${meta.min_cell} conversations is dropped before publishing.`,

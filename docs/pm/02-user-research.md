...
| Friction proxies (repeated requests, corrections, one-and-done) approximate where users did not get what they came for | Proxies are defined from transcript structure alone and require no code from the team | Proxies miss real friction or flag non-problems; the correlation is weak | Spot-check 30 flagged and 30 unflagged conversations by hand (about one hour) and count how often each proxy matches "did the user get what they came for"; the full 300-label procedure in research/friction-labeling-guide.md is the follow-up if the spot-check looks promising |

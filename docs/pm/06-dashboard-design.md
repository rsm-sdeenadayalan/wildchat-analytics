...
**Intent.** "What are people trying to do." A reliability card opens the view: held-out accuracy, the two-rater agreement the gate is tied to, and per-class F1, so a reader knows which class differences are real before reading a chart. A 100% stacked area shows intent share by week; two more 100% stacked bar sets repeat the same seven classes by model family and by the top ten languages, each with a legend. Colors follow data order per chart in v1, so a class is not guaranteed the same color across charts; a fixed taxonomy-order color domain is planned polish. Labeling has not run on this build, so the view shows a "not available" card instead.

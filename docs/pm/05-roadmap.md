...
**What this is for:** Show what Loupe ships now, what comes next, what is deferred, and what was cut on purpose, so scope discipline is as visible as scope.
**Date:** 2026-09-20
**Status:** Draft

## Now

Everything P0 and P1 from the PRD requirements table ships, over the full 3.2M-conversation corpus:
...
One gap inside "shipped": labeling and classification have not run, for lack of an API credential. The Intent view shows a "not available" card rather than fabricated numbers, and the friction precision table carries "pending" instead of real scores.
...
- Instrument the in-market metrics from PRD 6.1: export events, so exporting a number with its caveat counts as an answered question, plus a caveat retention check on exports.
- Build a second adapter for a common log format, OpenAI-compatible chat completion logs as JSONL, to prove the schema is not WildChat-specific.

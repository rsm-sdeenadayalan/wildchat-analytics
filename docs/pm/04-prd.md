...
| R4 | The full aggregate set regenerates from one committed command run against the cached flat tables. | P0 | `uv run python -m loupe.pipeline` (or the documented equivalent) reproduces `aggregates/*.parquet` and `aggregates/meta.json` with matching row counts. |
...
| R7 | A coverage banner states token-usage coverage window, intent-labeling coverage, and geography/language suppression share on the relevant views. | P1 | Banner text matches `aggregates/meta.json` fields (`token_coverage_first_week`, `intent_coverage`, suppression totals) on every load. |
...
- Whether intent classification should ship sample-only (the ~20,000-conversation labeled slice, with that scope stated) if the classifier fails the 85% held-out accuracy gate, rather than extending to the full corpus.
- Whether owners cannot answer usage questions without pulling an engineer into the work (assumption register, `02-user-research.md`).
- Whether owners judge quality primarily by reading transcripts by hand (assumption register, `02-user-research.md`).
- Whether a caveat attached to a number actually increases trust in it, or is ignored as noise (assumption register, `02-user-research.md`).
- Whether the target owners' decision cadence is weekly, as designed, or some other rhythm (assumption register, `02-user-research.md`).
- Whether intent is genuinely the metric family owners want first, ahead of friction or raw volume (assumption register, `02-user-research.md`).
- Whether the four friction proxies (repeated request, correction, refusal, one-and-done) actually track "did the user get what they came for" well enough to trust before the full 300-label validation runs (assumption register, `02-user-research.md`).
...
| 2026-09-20 | Label the intent sample with claude-haiku-4-5 under a $15 budget cap | claude-sonnet-5 or claude-opus-5, which cost more per token; an uncapped budget | The owner chose the cheapest model that supports the required JSON-schema output; estimated cost is about $9 for 20,000 conversations, leaving headroom under the $15 cap; thinking is explicitly disabled on thinking-capable models so thinking tokens cannot truncate the JSON output, and a 50-request canary must parse at 90% or higher before the main batch is submitted |

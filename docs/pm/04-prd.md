...
Labeling (Stage 3a) ran on 2026-09-25 through the UCSD TritonAI gateway (claude-sonnet-5): 9,829 of 10,000 sampled conversations labeled. Classification (Stage 3b) ran on 2026-09-26 with taxonomy v2 and passed the restated gate (78.6% held-out against 78.4%); `intent_coverage` is `full` and R1 and R3 are met for the intent view, with the reliability figures shown on the view itself.
...
| 2026-09-25 | Reduce the labeling sample from 20,000 to 10,000 conversations | Keep 20,000; cut to 5,000 | 5,000 would be almost entirely per-stratum floors; 10,000 keeps half the sample proportional and halves the gateway time |
| 2026-09-26 | Run an inter-rater study before changing anything: a second model labels 348 of the same conversations | Force the 72% classifier; publish sample-only intent; skip intent | Agreement was 83.6% on ten classes and 87.1% on seven, which set the ceiling any classifier could reach and located the ambiguous classes |
| 2026-09-26 | Revise the taxonomy to seven classes by merging information_seeking, homework_study, and personal_advice into questions, and writing_editing and business_professional into writing_and_business; v1 kept on record | Keep ten classes | The merged pairs are where the raters disagreed; provenance is recorded per class and v1 labels map forward without relabeling |
| 2026-09-26 | Restate the classifier gate as 90% of measured inter-rater agreement (78.4% for v2), with the absolute 85% as fallback when no rater study exists | Lower the bar to whatever the model reached; keep 85% and ship nothing | A gate above the label ceiling is unmeetable by construction; tying it to a measured ceiling keeps it principled and reproducible. The classifier passed at 78.6% without override |
